...
void Interpolate(const Pixel a, const Pixel b, std::vector<Pixel>& result)
{
	...
	for (int i = 0; i < N; ++i)
	{
		result[i] = Pixel { glm::round(current_pos), current_z_inv, current_camera_pos / current_z_inv, current_normal, current_diffuse_reflectance, current_indirect_reflectance };
